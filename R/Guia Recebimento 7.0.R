...
GR <- GR %>%
  ...
  fill(Referência, GR, Evento, `UG/Gestão`, Recolhedor, `Inscrição`, `Data Referência`) %>%

  select(Referência, `UG/Gestão`, Recolhedor,
...
GR_Final2 <- GR_Final %>% left_join(Estorno_nao_Conciliado,
                                    by = c("Referência", "UG/Gestão", "Recolhedor", "Inscrição", "Classificação",
...
Aviso <- data.frame(Atenção = c(
  ...
  "    São identificadas na coluna 'Estorno' junto aos seus pares, resultando em soma 0 (quando não é estorno parcial);",
