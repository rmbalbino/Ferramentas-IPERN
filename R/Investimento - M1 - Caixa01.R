...
# PDF <- pdf_text("CAIXA FIC PRATICO RENDA FIXA CURTO.pdf")[[1]]
...
Caixa01 <- function(PDF){
  ...
  PDF <- PDF %>% 
    ...
    mutate(FUNDO = ifelse(str_detect(Dados, "^Fundo"), str_remove(lead(Dados), "\\d{2}\\.\\d{3}.*"), NA), 
           
           
           CONTA = ifelse(str_detect(Dados, "Conta Corrente"), paste0(str_sub(str_remove(lead(Dados, n = 2), "\\s*"), -4), 
                                                                      str_sub(lead(Dados, n = 4), -1)), NA), 
           
           
           CNPJ = ifelse(str_detect(Dados, "^Fundo"), str_extract(lead(Dados), "\\d{2}\\.\\d{3}\\.\\d{3}\\/\\d{4}-\\d{2}"), NA), 
           ...
           `RENTABILIDADE (%)` = ifelse(str_detect(Dados, "No Ano"), str_extract(lead(str_trim(Dados), n = 2), "\\d{1},\\d{2,}-|\\d{1},\\d{2,}"), NA), 
           ...
           COTA = ifelse(str_detect(Dados, "No Ano"), str_extract(lead(str_trim(Dados), n = 2), "\\d{1},\\d{2,}$"), NA),
